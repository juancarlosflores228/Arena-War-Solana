    ...
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-arena-border/60 bg-arena-bg/80 backdrop-blur-xl">
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded border border-arena-green/30 bg-arena-green/5">
            ...
            <span className="font-body text-xs text-arena-green tracking-wider">{t.nav.devnet}</span>
          </div>

          <motion.button
            whileTap={{ scale: 0.93 }}
            onClick={toggle}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded border border-arena-border hover:border-arena-red/40 bg-arena-surface transition-colors"
          >
            <span className="text-sm leading-none">{lang === 'es' ? '🇲🇽' : '🇺🇸'}</span>
            <span className="font-display text-xs font-bold tracking-widest text-arena-muted">
              {lang === 'es' ? 'ES' : 'EN'}
            </span>
          </motion.button>
      ...
      {menuOpen && (
        ...
        >
          {NAV_LINKS.map(({ label, path }) => (
            <Link
              ...
              className={`font-display text-sm tracking-widest px-3 py-2 rounded ${
                location.pathname === path ? 'text-arena-red bg-arena-red/10' : 'text-arena-muted'
              }`}
            >
              {label}
            </Link>
          ...
          <button
            onClick={toggle}
            className="text-left font-display text-sm tracking-widest px-3 py-2 text-arena-muted"
          >
